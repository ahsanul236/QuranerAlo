(function(){
  ...
  function printExpense(id){
    ...
    const html = `<!doctype html><html lang="bn"><head><meta charset="utf-8"><title>${esc(e.expenseNo)}</title><style>
      @page{size:A4;margin:15mm}body{font-family:Arial,"Noto Sans Bengali",sans-serif;color:#172033;margin:0}.wrap{max-width:760px;margin:auto}.head{display:flex;align-items:center;gap:18px;border-bottom:2px solid #c9a33b;padding-bottom:14px}.logo{width:90px;height:90px;object-fit:contain}.brand h1{margin:0;font-size:25px}.brand p{margin:4px 0;color:#555;font-size:13px}.meta{margin-top:18px;display:flex;justify-content:space-between;gap:20px}.box{border:1px solid #ddd;border-radius:10px;padding:12px}.title{font-size:22px;font-weight:700;text-align:center;margin:24px 0 14px}.table{width:100%;border-collapse:collapse}.table th,.table td{border:1px solid #ddd;padding:10px}.table th{background:#f6f1df;text-align:left;width:35%}.amount{font-size:24px;font-weight:700;text-align:right;margin-top:18px}.sign{display:flex;justify-content:space-between;margin-top:85px}.sign div{width:30%;text-align:center;border-top:1px solid #777;padding-top:6px;font-size:12px}
    </style></head><body><div class="wrap"><div class="head">${logo ? `<img class="logo" src="${logo}" alt="Logo">` : ''}<div class="brand"><h1>${esc(s.institutionName)}</h1><p>${esc(s.subtitle)}</p><p>${esc(s.address||'')} ${s.phone ? '• '+esc(s.phone) : ''}${s.email ? ' • '+esc(s.email) : ''}</p></div></div><div class="title">অন্যান্য ব্যয়ের ভাউচার / EXPENSE INVOICE</div><div class="meta"><div class="box"><b>ভাউচার নং:</b> ${esc(e.expenseNo)}</div><div class="box"><b>তারিখ:</b> ${esc(e.date)}</div></div><table class="table" style="margin-top:16px"><tr><th>খরচের শিরোনাম</th><td>${esc(e.title)}</td></tr><tr><th>খরচের ধরন</th><td>${esc(e.category)}</td></tr><tr><th>যাকে/যেখানে প্রদান</th><td>${esc(e.payee||'—')}</td></tr><tr><th>পেমেন্ট মাধ্যম</th><td>${esc(e.method||'—')}</td></tr><tr><th>রেফারেন্স/বিল নং</th><td>${esc(e.reference||'—')}</td></tr><tr><th>বিস্তারিত</th><td>${esc(e.note||'—')}</td></tr></table><div class="amount">মোট খরচ: ${money(e.amount)}</div><div class="sign"><div>প্রদানকারী / গ্রহণকারী</div><div>হিসাব রক্ষক</div><div>পরিচালক / কর্তৃপক্ষ</div></div></div><script>window.onload=()=>setTimeout(()=>window.print(),250);</script></body></html>`;
  ...
  function showExpenses(){
    ['dashboard','students','payments','staffs','monthly-sheet','invoice','sheets-guide','settings'].forEach(t=>{
  ...
  function render(){
    ...
    document.getElementById('ka-exp-all-total').textContent = money(window.state.expenses.reduce((a,x)=>a+Number(x.amount||0),0));
  ...
  function resetForm(){
    ...
    const date = document.getElementById('ka-exp-date'); if(date) date.value=today();
    const method = document.getElementById('ka-exp-method'); if(method) method.value='নগদ';
    const cat = document.getElementById('ka-exp-category'); if(cat) cat.value='অফিস / পরিচালনা';
    const id = document.getElementById('ka-exp-id'); if(id) id.value='';
    const title = document.getElementById('ka-exp-form-title'); if(title) title.textContent='নতুন অন্যান্য ব্যয় এন্ট্রি';
    const saveBtn = document.getElementById('ka-exp-save'); if(saveBtn) saveBtn.innerHTML='<i class="fa-solid fa-floppy-disk"></i> খরচ সেভ করুন';
  ...
  function editExpense(id){ ensureState(); const x=window.state.expenses.find(a=>a.id===id); if(x) fillExpense(x); }

  function deleteExpense(id){
    ensureState(); const x=window.state.expenses.find(a=>a.id===id); if(!x) return;
  ...
  function submit(e){
    ...
    if(!title || !date || amount<=0){ alert('খরচের শিরোনাম, তারিখ ও সঠিক খরচের পরিমাণ দিন।'); return; }
    const old=id ? window.state.expenses.find(x=>x.id===id) : null;
    const payload={
      id:id||uid(), expenseNo:old?.expenseNo||uid(), title,
      category:document.getElementById('ka-exp-category').value.trim()||'অন্যান্য', amount, date,
  ...
  function addUI(){
    ...
    if(window.KA_MEMBER && window.KA_MEMBER.status && window.KA_MEMBER.status !== 'active') return;
    ...
    document.querySelector('[data-ka-exp-logo]').src = (window.state.siteSettings && window.state.siteSettings.logoUrl) || window.KA_DEFAULT_LOGO || '';
    resetForm(); render();
  }

  function boot(){ patchNormalizeState(); ensureState(); addUI(); wrapSwitchTab(); render(); }
