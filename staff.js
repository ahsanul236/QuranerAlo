...
function code(prefix){return `${prefix}-${new Date().toISOString().replace(/[-:TZ.]/g,'').slice(0,14)}`;}
...
async function load(){
 ...
 const fields=mode==='teacher'?'teacher_id,teacher_code,full_name,full_name_bn,phone,email,specialization,joining_date,active':'staff_id,staff_code,full_name,phone,email,staff_type,joining_date,active';
...
$('staffForm').addEventListener('submit',async e=>{e.preventDefault();msg('Saving…');const f=new FormData(e.currentTarget);if(mode==='teacher'){const {error}=await supabase.from('qa_teachers').insert({teacher_code:code('TCH'),full_name:f.get('full_name').trim(),full_name_bn:f.get('full_name_bn')?.trim()||null,phone:f.get('phone').trim(),email:f.get('email').trim(),specialization:f.get('specialization')?.trim()||null,joining_date:f.get('joining_date')||null,notes:f.get('notes').trim()});if(error){msg('Teacher record save করা যায়নি।','error');return;}}else{const {error}=await supabase.from('qa_staff').insert({staff_code:code('STF'),full_name:f.get('full_name').trim(),staff_type:f.get('staff_type'),phone:f.get('phone').trim(),email:f.get('email').trim(),joining_date:f.get('joining_date')||null,notes:f.get('notes').trim()});if(error){msg('Staff record save করা যায়নি।','error');return;}}e.currentTarget.reset();e.currentTarget.mode.value=mode;msg('Record সফলভাবে save হয়েছে।','success');await load();});
