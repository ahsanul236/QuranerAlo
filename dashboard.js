...
async function logEvent(action, targetUserId = null, meta = {}) {
  await supabase.from('qa_audit_log').insert({
...
supabase.auth.onAuthStateChange((event, session) => {
  if ((event === 'SIGNED_OUT' || !session) && document.visibilityState !== 'hidden') redirectToLogin();
});

init().catch((error) => {
  console.error(error);
  $('loading').textContent = 'Dashboard load করা যায়নি। আবার login করুন।';
});
