(function(){
  ...
  function add(role,text){
    ...
    const div=document.createElement('div');div.className='qaa-msg '+(role==='user'?'qaa-user':'qaa-ai');
  ...
  function show(){
    ...
    root.innerHTML=`
      <button id="qaa-fab" aria-label="Quraner Alo admin সহায়তা"><span>✦</span> Admin সহায়তা</button>
      <section id="qaa-panel" aria-label="Quraner Alo admin সহায়তা">
        <header class="qaa-head"><div><strong>Quraner Alo Admin সহায়তা</strong><small>শুধু Active Admin-এর জন্য • বিনামূল্যে</small></div><button id="qaa-close">×</button></header>
        <div id="qaa-messages" class="qaa-messages"><div class="qaa-msg qaa-ai"><div class="qaa-bubble">আসসালামু আলাইকুম। আপনার সিস্টেমে যে সমস্যাটি দেখছেন, সেটি বাংলায় লিখুন। আমি পরিচিত সমস্যার সমাধান এক ধাপ করে দেখাব।</div></div></div>
    ...
    const st=document.createElement('style');st.textContent=`
      ...
      .qaa-head{background:#0f766e;color:#fff;padding:14px 16px;display:flex;align-items:center;justify-content:space-between}.qaa-head strong{display:block;font-size:15px}.qaa-head small{opacity:.88;font-size:11px}.qaa-head button{border:0;background:transparent;color:#fff;font-size:24px;cursor:pointer}
    ...
    document.getElementById('qaa-fab').onclick=()=>{document.getElementById('qaa-fab').style.display='none';document.getElementById('qaa-panel').style.display='flex';document.getElementById('qaa-panel').style.flexDirection='column';document.getElementById('qaa-input')?.focus()};
  ...
  async function send(e){
    ...
    add('user',text);history.push({role:'user',content:text});input.value='';btn.disabled=true;add('ai','সমাধান খুঁজছি…');
    const pending=[...document.querySelectorAll('#qaa-messages .qaa-msg')].pop();
    try{
      const token=window.KORANER_ACCESS_TOKEN;
      const r=await fetch(FN,{method:'POST',headers:{apikey:K,Authorization:'Bearer '+token,'Content-Type':'application/json'},body:JSON.stringify({message:text,history:history.slice(-10),context:currentContext()})});
      const j=await r.json();pending?.remove();
      if(j.error==='Admin access required'){add('ai','এই সহায়তাটি শুধু Active Admin-এর জন্য উপলব্ধ।');return;}
      const answer=j.answer||'এই সমস্যার জন্য এখনো কোনো নির্দেশনা পাওয়া যায়নি।';add('ai',answer);history.push({role:'assistant',content:answer});
    }catch{pending?.remove();add('ai','সহায়তা সেবা এখন পাওয়া যাচ্ছে না। একটু পরে আবার চেষ্টা করুন।');}
    finally{btn.disabled=false;input.focus()}
  }

  async function boot(){if(await isAdmin())show();}
