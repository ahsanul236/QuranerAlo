...
function guardianHtml(g){
  ...
  <div class="profile-field"><label>Guardian হিসেবে</label><select data-g-field="relation" data-g-id="${esc(g.guardian_id)}" ${dis?'disabled':''}><option value="">নির্বাচন করুন</option><option value="Father" ${g.relation==='Father'?'selected':''}>Father</option><option value="Mother" ${g.relation==='Mother'?'selected':''}>Mother</option><option value="Other" ${g.relation==='Other'?'selected':''}>Other</option></select></div>
